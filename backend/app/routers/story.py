...
@router.post('/yunsuan')
async def yun_suan(payment_token: schemas.CompletedPayment, background_tasks: BackgroundTasks):
    # Initial validation
    ...
    result = await process_yunsuan(payment_token)
    result = result.model_dump(mode="json")
    
    return result

async def process_yunsuan(payment_token: schemas.CompletedPayment):
    try:
...
@router.post("/tuisuan")
async def tui_suan(payment_token: schemas.CompletedPayment, background_tasks: BackgroundTasks):
    # Initial validation
    payment_complete, lack = database.check_payment(payment_token.user_id, 
                                payment_token.session_id, 
                                payment_token.order_id)
    ...
    result = await process_tuisuan(payment_token)
    result = result.model_dump(mode="json")
    
    return result

async def process_tuisuan(payment_token: schemas.CompletedPayment):
    try:
